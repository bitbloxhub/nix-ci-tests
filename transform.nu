  ...
  | each {|status|
    match $status.type {
        "EVAL" => (if ($status.success) {
          ...
          $status.attr | inspect
          let eval_err = (do -i
            {script -efq -c $"nix eval --show-trace \".#checks.($system).($status.attr)\"" e+o>| sed "s/\\\[x1b2026h*\\\[x1b2026l//"}
          )
          rm ./typescript
        ...
        "BUILD" => {
          $status.attr | inspect
          let build_log = (do -i
            {script -efq -c $"nix log \".#checks.($system).($status.attr)\"" e+o>| sed "s/\\\[x1b2026h*\\\[x1b2026l//"}
          )
          rm ./typescript
